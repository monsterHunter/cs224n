...
def pad_sents_char(sents, char_pad_token):
    ...
    #
    # for s in sents:
    #     padded = [[char_pad_token] * max_word_length] * max_sent_len
    #     padded[:len(s)] = s
    #     for w in padded:
    #         if len(w) > max_word_length:
    #             del w[max_word_length:]
    #         elif len(w) < max_word_length:
    #             w += ([char_pad_token] * (max_word_length - len(w)))
    ...
    for s in sents:
        padded = [[char_pad_token] * max_word_length] * max_len
        for i, w in enumerate(s):
            w=w[:min(max_word_length,len(w))]
            padded[i] = w + [char_pad_token]*(max_word_length-len(w))
        sents_padded.append(padded)
